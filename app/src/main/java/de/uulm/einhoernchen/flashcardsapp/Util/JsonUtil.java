...
//		Json.setObjectMapper(mapper);
        return null; //@TODO to be implemented Json.toJson(o);
    }
    ...
    public static ObjectNode prepareJsonStatus(int statuscode,
                                               String description) {
        ObjectNode result = null; //@TODO to be implemented Json.newObject();
    ...
    public static List<Answer> retrieveAnswers(JsonNode json) {
        ...
        for (JsonNode node : answersNode) {
            ...
            if (node.has(JsonKeys.ANSWER_ID)) {
                Answer found = null; // Answer.find.byId(node.get(JsonKeys.ANSWER_ID).asLong());
    ...
    public static AnswerRating parseAnswerRating(JsonNode json) {
        ...
        if(json.has(JsonKeys.AUTHOR)){
            author = null; //@TODO to be implemented User.find.byId(json.get(JsonKeys.AUTHOR).get(JsonKeys.USER_ID).asLong());
            System.out.println("Rating user="+author);

        }
        if(json.has(JsonKeys.ANSWER)){
            answer = null; //@TODO to be implemented Answer.find.byId(json.get(JsonKeys.ANSWER).get(JsonKeys.ANSWER_ID).asLong());
    ...
    public static CardRating parseCardRating(JsonNode json) {
        ...
        if(json.has(JsonKeys.FLASHCARD)){
            flashCard = null; //@TODO to be implemented FlashCard.find.byId(json.get(JsonKeys.FLASHCARD).get(JsonKeys.FLASHCARD_ID).asLong());
